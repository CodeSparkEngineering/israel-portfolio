...
/**
 * Entrance animation for the hero (`.hero-reveal` in globals.css). Plain CSS instead of
 * FadeIn so the content is visible on first paint, before the JavaScript loads.
 */
const reveal = (delaySeconds: number, fromY: number): CSSProperties =>
  ({ '--reveal-delay': `${delaySeconds}s`, '--reveal-y': `${fromY}px` }) as CSSProperties
...
export default function HeroSection({ locale, dict }: HeroSectionProps) {
  ...
  return (
    ...
    >
      ...
      <nav className="hero-reveal w-full px-6 md:px-10 pt-6 md:pt-8 z-20" style={reveal(0, -20)}>
        <ul className="flex flex-wrap items-center justify-between w-full gap-x-4 gap-y-3">
          {nav.map((link) => (
            <li key={link.href}>
              {/* py-3 -my-3: larger tap target without changing the layout */}
              <a
                href={link.href}
      ...
      <div className="w-full overflow-hidden">
        <h1
          className="hero-reveal hero-heading w-fit mx-auto font-black uppercase tracking-tight leading-none whitespace-nowrap text-center mt-8 sm:mt-4 md:-mt-5 text-[calc(13vw*var(--title-scale))] sm:text-[calc(12vw*var(--title-scale))] md:text-[calc(12.5vw*var(--title-scale))] lg:text-[calc(13.5vw*var(--title-scale))]"
          style={{ ...reveal(0.15, 40), '--title-scale': hero.titleScale } as CSSProperties}
        >
          {hero.title}
        </h1>
      </div>
      ...
      <div
        className="hero-reveal relative mx-auto mt-6 w-[300px] z-10 sm:absolute sm:mt-0 sm:left-1/2 sm:-translate-x-1/2 sm:bottom-0 sm:w-[360px] md:w-[440px] lg:w-[520px]"
        style={reveal(0.6, 30)}
      >
        <Magnet
          padding={200}
      ...
      <div className="mt-auto w-full px-6 md:px-10 pt-8 pb-8 sm:pt-0 sm:pb-8 md:pb-10 flex flex-col items-start gap-6 sm:flex-row sm:justify-between sm:items-end z-20">
        <p
          className="hero-reveal text-[#D7E2EA] font-light uppercase tracking-wide leading-snug max-w-[300px] sm:max-w-[260px] md:max-w-[320px] lg:max-w-[440px]"
          style={{ ...reveal(0.35, 20), fontSize: 'clamp(0.75rem, 1.4vw, 1.5rem)' }}
        >
          {hero.tagline}
        </p>

        <div className="hero-reveal" style={reveal(0.5, 20)}>
